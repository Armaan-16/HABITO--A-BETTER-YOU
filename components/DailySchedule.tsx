...
import { Sparkles, ChevronLeft, ChevronRight, Circle, CheckCircle2, Calendar, Activity, List, TrendingUp, Plus, Minus, LayoutGrid } from './Icons';
...
const DailySchedule: React.FC<DailyScheduleProps> = ({ scheduleData, setScheduleData }) => {
  ...
  // Sync calendar view when selected date changes externally
  useEffect(() => {
  ...
  return (
    <div className="bg-surface rounded-3xl p-6 border border-surfaceHighlight shadow-xl h-full flex flex-col transition-all duration-300">
      {/* Header & Nav */}
      <div className="flex items-center justify-between mb-4">
        <div>
            <h2 className="text-xl font-bold text-white font-sans">Plan Your Day</h2>
        ...
        <div className="flex items-center gap-2">
             {/* Toggle Calendar Grid */}
            <button 
                onClick={() => setShowCalendar(!showCalendar)}
                className={`p-2 rounded-xl transition-all border ${showCalendar ? 'bg-primary text-white border-primary' : 'bg-surfaceHighlight/30 text-gray-400 hover:text-white border-white/5'}`}
      ...
      {showCalendar ? (
          // --- CALENDAR GRID VIEW ---
          <div className="flex-1 bg-surfaceHighlight/10 rounded-2xl p-4 border border-white/5 animate-in zoom-in-95 overflow-hidden flex flex-col">
            <div className="flex items-center justify-between mb-4">
            ...
            <div className="grid grid-cols-7 gap-1 flex-1 overflow-y-auto custom-scrollbar">
                {calendarDays.map((date, i) => {
                    ...
                    // Check if this date has any active tasks
                    const dayItems = scheduleData[dKey] || [];
                    const activeCount = dayItems.filter(x => x.activity?.trim()).length;
                    const isSelected = getLocalDateKey(selectedDate) === dKey;
                    const isTodayDate = getLocalDateKey(new Date()) === dKey;

                    return (
                        <button
                            key={dKey}
                            onClick={() => selectCalendarDate(date)}
                            className={`relative p-2 rounded-xl flex flex-col items-center justify-center min-h-[44px] transition-all border ${
                                isSelected 
                        ...
                        >
                            ...
                            {activeCount > 0 && (
                                <div className={`w-1 h-1 rounded-full mt-1 ${isSelected ? 'bg-white' : 'bg-success'}`} />
                            )}
